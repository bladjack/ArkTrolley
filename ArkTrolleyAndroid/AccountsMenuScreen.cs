...
//using ArkTrolleyCore.LocalStorage;
...
{
	[Activity (Label = "The Ark Trolley for Android: Accounts management", MainLauncher = false, Icon = "@drawable/arkIcon", ScreenOrientation = ScreenOrientation.Portrait)]
	public class AccountsMenuScreen : Activity
	{
		LocalUserClass[] obj_LocalUsers = new LocalUserClass[]{};
		...
		{ //Method for Startup initializations: 
			...
			Log.Debug (GetType().FullName, "Log Debug: AccountsMenuScreen OnCreate");

			if (bundle != null)
			{//This is the same bundle provided to OnRestoreInstanceState but it is better to wait until all the UI and other steps
				//are completed before restore the saved state, thats why it is better to use OnRestoreInstanceState instead.
			...
			// Set our view from the "AccountsMenuScreen" layout resource
		...
		{//This is called after the OnCreate method is finished, and provides another opportunity for an Activity to restore its state after initialization is complete.
			...
			Log.Debug (GetType().FullName, "Log Debug: AccountsMenuScreen OnRestoreInstanceState");

			str_User_Name = savedState.GetString("str_User_Name");
			str_User_Email = savedState.GetString("str_User_Email");
		}
		...
		{//Method to perform any specific tasks right before an activity becomes visible such as refreshing current values of views within the activity
			...
			Log.Debug (GetType().FullName, "Log Debug: AccountsMenuScreen OnStart");
			...
			LocalStorage obj_LocalStorage = new LocalStorage();
			obj_LocalUsers = obj_LocalStorage.getLocalUsers ();

			PresentationClass obj_Presentation = new PresentationClass(this);
			obj_Presentation.buildUsersMenu (obj_LocalUsers);
		}
		...
		{//The system calls this method when the Activity is ready to start interacting with the user
			...
			Log.Debug (GetType().FullName, "Log Debug: AccountsMenuScreen OnResume");
		}
		...
		{//Called when the system is about to put the activity into the background or when the activity becomes partially obscured
			...
			Log.Debug (GetType().FullName, "Log Debug: AccountsMenuScreen OnPause");
		}
		...
		{//This method is called when the activity is no longer visible to the user.
			...
			Log.Debug (GetType().FullName, "Log Debug: AccountsMenuScreen OnStop");
		...
		{//This method is called after your activity has been stopped, prior to it being started again.
			...
			Log.Debug (GetType().FullName, "Log Debug: AccountsMenuScreen OnRestart");
		...
		{//This is invoked by Android when the activity is being destroyed. Activities can implement this method if they need to persist any key/value state items.
			outState.PutString ("str_User_Name", str_User_Name);
			outState.PutString ("str_User_Email", str_User_Email);

			base.OnSaveInstanceState (outState);
			Log.Debug (GetType().FullName, "Log Debug: AccountsMenuScreen OnSaveInstanceState");
		}
		...
		{//This is the final method that is called on an Activity instance before it’s destroyed and completely removed from memory.
			...
			Log.Debug (GetType().FullName, "Log Debug: AccountsMenuScreen OnDestroy");
