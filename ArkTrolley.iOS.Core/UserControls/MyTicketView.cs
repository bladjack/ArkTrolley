...
{
	public class MyTicketView: UIView {
		// control declarations
		ProfileViewModel pageViewModel;
		private static MyTicketView SharedInstance;
		private  string LoginUserName;
		private  string StoreName;
		private List<MyTicketsModel> myListData;
		UIScrollView scrollView;// new UIScrollView ();

		private double totalCost=0;
	
		private MyTicketView (RectangleF frame, string user, string store, List<MyTicketsModel> data, ProfileViewModel pageViewModel) : base (frame)
		{
			// configurable bits
		...
		private async void GetMyTicketDetail(string int_id)
		{
			await pageViewModel.GetMyTicketDetailData(int_id);
			this.Hide ();
		}
		...
		private async void DeleteItemFromList(MyTicketsModel item)
		{
		...
		public static MyTicketView Show(RectangleF bounds, string user, string store, List<MyTicketsModel> data, ProfileViewModel viewModel)
		{
			SharedInstance = new MyTicketView (bounds,user,store,data,viewModel);
