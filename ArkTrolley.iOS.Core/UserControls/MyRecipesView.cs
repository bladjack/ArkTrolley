...
{
	public class MyRecipesView: UIView {
		// control declarations
		ProfileViewModel pageViewModel;
		private static MyRecipesView SharedInstance;
		...
		private MyRecipesView (RectangleF frame, string user, string store, List<MyRecipesModel> data, ProfileViewModel pageViewModel) : base (frame)
		{
			// configurable bits
		...
		private void AddUserAndStore()
		{
			var userLabel = new UILabel {
				Text = SharedData.CurrentConfigurationData.str_my_recipes_text,
		...
		private async void AddStoreList(float topmargin, float bottomargin)
		{
			...
			foreach (var item in myListData) {
				...
				newUser1.AddGestureRecognizer (new UITapGestureRecognizer (() => {
					pageViewModel.GetMyRecipesDetailData(item.int_id);
		...
		public static MyRecipesView Show(RectangleF bounds, string user, string store, List<MyRecipesModel> data, ProfileViewModel viewModel)
		{
			SharedInstance = new MyRecipesView (bounds,user,store,data,viewModel);
