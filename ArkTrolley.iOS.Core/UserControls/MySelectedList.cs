...
{
	public class MySelectedList: UIView {
		// control declarations

		private static MySelectedList SharedInstance;
		private  string LoginUserName;
		private  string StoreName;
		private List<MyListDetailModel> myselectedlist;
		UIScrollView scrollView;// new UIScrollView ();

		private double totalCost=0;
		ProfileViewModel pageViewModel;

		private MySelectedList (RectangleF frame, string user, string store, List<MyListDetailModel> data,ProfileViewModel pageViewModel) : base (frame)
		{
			// configurable bits
		...
		private async Task AddBackGroundAndTitle()
		{
			...
			var image = await ArkTrolley.iOS.Core.AppHelper.UtilityMethods.LoadImage (ConfigurationSettings.GetSelectedListTitleImagePath);
		...
		private async void AddStoreList(float topmargin, float bottomargin)
		{
			...
			var totalCostLabel = new UILabel {
				Text = SharedData.CurrentConfigurationData.str_selected_list_text +" "+ totalCost.ToString() ,
				TextColor = CustomUIColor.FromHexString ("#000000"),
		...
		private async Task<UIView> StoreHub(UIImage userImage, MyListDetailModel item)
		{
		...
		private async void DeleteItemFromList(MyListDetailModel item)
		{
			...
			if (button == ArkTrolley.Core.AppHelper.AppEnums.DialogButton.Button1) {
				var response = await pageViewModel.SetRemoveListItem (item.int_id, ListType.mylists, item.int_id);
		...
		public static MySelectedList Show(RectangleF bounds, string user, string store, List<MyListDetailModel> data,ProfileViewModel pageViewModel)
		{
			SharedInstance = new MySelectedList  (bounds,user,store,data,pageViewModel);
